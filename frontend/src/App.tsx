...
import { matchRoute, useHashRouter } from "./hooks/useHashRouter";
...
const App: React.FC = () => {
  ...
  function renderPage() {
    ...
    console.log("🔍 Bucket wildcard match:", {
    ...
    console.log("🔍 Bucket exact match:", {
      pattern: "/buckets/:bucket",
    ...
    if (exactUploadMatch) {
      return (
        <UploadPage bucket={exactUploadMatch.bucket} onNavigate={navigate} />
      );
    }
  ...
  return (
    <ToastProvider>
      <Layout onNavigate={navigate}>{renderPage()}</Layout>
      <ToastContainer />
    </ToastProvider>
  );
};
